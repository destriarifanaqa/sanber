class LoginClass{
    ...
    interceptUnsuccess(){
        cy.intercept('POST', 'https://opensource-demo.orangehrmlive.com/web/index.php/auth/validate').as('authVal')
    }
    clickLoginBTN(){
        cy.get('.oxd-button').should('be.visible').click()
    }
    waitSuccess(){
        cy.wait('@actSum').its('response.statusCode').should('eq', 200)
    }
    waitUnsuccess(){
        cy.wait('@authVal').its('response.statusCode').should('eq', 302)
    }
    assertionLogin(){
